  ...
  $(".myFavourites").click(function () {
    ...
    if (films.length > 0) {
      ...
      for (i = 0; i < films.length; i++) {
        axios
          .get("https://www.omdbapi.com?apiKey=" + apiKey + "&s=" + films[i])
          .then((response) => {
...
function getMovies(searchText) {
  ...
  if (searchText.length > 2) {
    axios
      .get("https://www.omdbapi.com?apiKey=" + apiKey + "&s=" + searchText)
      .then((response) => {
...
$(document).on("click", ".fa-star", function () {
  if ($(this).attr("id") == "not-checked") {
    ...
    films.push(title);
    $("#favouriteLinks").html(
      '&nbsp;Favourites&nbsp;<i class="fas fa-star" id="default"></i>' +
        films.length
    );
  } else {
    $(this).removeAttr("id");
    ...
    $("#favouriteLinks").html(
      '&nbsp;Favourites<i class="fas fa-star" id="default"></i>' + films.length
    );
  }
